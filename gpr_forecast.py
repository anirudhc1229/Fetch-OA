...
def combined_kernel(X1, X2, params):
    ...
    return linear_kernel(X1, X2, params[0]) + periodic_kernel(X1, X2, params[1], params[2], params[3])
...
def negative_log_likelihood(params, X, y):
    ...
    K = combined_kernel(X, X, params) + params[4] * np.eye(len(X))  # Add noise variance to diagonal
    L = cholesky(K, lower=True)  # Perform Cholesky decomposition for numerical stability
    alpha = cho_solve((L, True), y)  # Solve for alpha in K*alpha = y
    return 0.5 * np.dot(y.T, alpha) + np.sum(np.log(np.diag(L))) + 0.5 * len(X) * np.log(2 * np.pi)
...
class GaussianProcessRegression:
    ...
    def predict(self, X_test):
        ...
        K = combined_kernel(self.X_train, self.X_train,
                            self.params) + self.params[4] * np.eye(len(self.X_train))
        L = cholesky(K, lower=True)
        alpha = cho_solve((L, True), self.y_train)
        
        K_star = combined_kernel(self.X_train,
                                 X_test,
                                 self.params)  # Covariance between training and test data
        f_mean = np.dot(K_star.T,
                        alpha)  # Predicted mean
        
        v = cho_solve((L,
                       True),
                      K_star)
        f_var = combined_kernel(X_test,
                                X_test,
                                self.params) - np.dot(K_star.T,
...
def prepare_data():
    ...
    return X_normalized ,y_normalized.flatten() ,X_mean ,X_std ,y_mean ,y_std.flatten() ,monthly_data

def generate_forecast():
    """Generates forecast for future time periods using the trained Gaussian Process model."""

    # Prepare normalized training data
    X_normalized,y_normalized,X_mean,X_std,y_mean_flattened,y_std_flattened ,monthly_data= prepare_data()
    ...
    y_pred_normalized,y_var_normalized= model.predict(X_test_normalized.reshape(- 1 ,1))

    # Denormalize predictions to original scale
    y_pred= y_pred_normalized*y_std_flattened+y_mean_flattened
    confidence_interval_upper_bound=(y_pred_normalized+1.96*np.sqrt(y_var_normalized))*y_std_flattened+y_mean_flattened
    confidence_interval_lower_bound=(y_pred_normalized-1.96*np.sqrt(y_var_normalized))*y_std_flattened+y_mean_flattened
    ...
    predictions=[]
    for date,prediction ,lower_bound ,upper_bound in zip(future_dates,y_pred.flatten() ,
